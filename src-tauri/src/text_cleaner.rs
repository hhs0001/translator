        ...
        for part in parts {
            ...
            if config.preserve_basic_formatting {
                for basic_tag in BASIC_FORMATTING_TAGS {
                    if trimmed == *basic_tag || trimmed.starts_with(&format!("{}\\ ", *basic_tag)) {
                        should_keep = true;
                        break;
                    }
                }
    ...
    let should_skip = style
        .map(|s| {
            config
                .ignored_styles
                .iter()
                .any(|ignored| s.to_lowercase().contains(&ignored.to_lowercase()))
        })
        .unwrap_or(false);
...
pub fn reapply_tags(
    ...
    // Se não há tags no resultado mas haviam no original,
